...
- Las API keys viven exclusivamente en `local.properties` (ignorado por git).
- `BuildConfig` las inyecta en tiempo de build vía `buildConfigField`, sin hardcodearlas en el source.
- Si una key llega a quedar expuesta accidentalmente, **rotarla es la única mitigación válida** — borrarla del historial no la des-compromete.
